...
<mappingDir>/MyProfile/
  ...
  PoKeysHardwareSupportModule.config   ← authored from p.drivers.pokeys when PoKeys is
                                         declared. Multi-device by serial. Each <Device> has
                                         <Serial>, optional <Name>, <PWMPeriodMicroseconds>
                                         (per-device since hardware shares the period
                                         across all 6 PWM channels), and three output lists:
                                         <DigitalOutputs>/<Output>{<Pin>,<Invert>},
                                         <PWMOutputs>/<Output>{<Channel>}, and
                                         <PoExtBusOutputs>/<Output>{<Bit>,<Invert>} (1..80
                                         flat bit numbering across the daisy-chained
                                         shift-register chain — bit 1 = Device 1 output A,
                                         bit 8 = Device 1 output H, bit 9 = Device 2
                                         output A, etc., matching the PoKeys vendor tool's
                                         labelling). Single editor driver id `pokeys`; the
                                         board exposes both digital and analog kinds, so
                                         the kind-mismatch validator dispatches per-channel
                                         via DRIVER_CHANNEL_KIND.pokeys (a function that
                                         returns 'analog' for PWM[N] and 'digital'
                                         otherwise) instead of a flat driver→kind value.
                                         Root is <PoKeys>.
```
...
2. **Hardware Config** — deep editor for per-driver `.config` files, one collapsible card per declared driver. **All ten drivers** (AnalogDevices, HenkSDI, HenkQuadSinCos, PHCC, ArduinoSeat, TeensyEWMU, TeensyRWR, TeensyVectorDrawing, NiclasMorinDTS, PoKeys) have structured editors today (no stub cards remain).
   ...
   - **PoKeys**: multi-device driver. One catalog id (`pokeys`) covering all three output kinds the board exposes — digital pins, PWM channels, PoExtBus relay bits. The kind-mismatch validator dispatches per-channel via `DRIVER_CHANNEL_KIND.pokeys` (a function — see catalogs.js's `getChannelKind`) since the board mixes analog and digital outputs. Per-device editor: Identity (Serial as the device address, optional Name, PWM Period in microseconds), a list of digital-pin outputs (Pin 1..55 + Invert checkbox; pin 54 is the device reset line — selectable but flagged in the dropdown so users don't accidentally wire it), a list of PWM channels (PWM1..PWM6, where PWM1=physical pin 17 … PWM6=physical pin 22 — labelled with both the PWM number and the physical pin so users wiring relays/RC servos pick the right output without referencing the manual), and a list of PoExtBus relay bits (Bit 1..80, labelled "Device N : letter (bit M)" matching the PoKeys vendor tool's daisy-chain layout, plus Invert checkbox). Pin/channel/bit dropdowns disable already-used values on the same device. The C# HSM (vendored alongside `lightningstools/src/SimLinkup/HardwareSupport/PoKeys/lib/PoKeysDevice_DLL.dll` v2025-11-04) is USB-only and addresses devices by serial — boards declared in config but not currently plugged in are logged and skipped, so a single missing PoKeys doesn't break unrelated outputs. PoExtBus is initialised the first time any bit is declared on a device (`AuxilaryBusSetData(1, zeros)`) and signal-changed handlers update one bit in a 10-byte cache then re-push the whole array (DLL has no per-bit API). Out-of-scope-for-v1 features documented inline in `PoKeysHardwareSupportModule.cs`: matrix LED, LCD, PoNET, digital counters, encoders, analog inputs, Ethernet/network discovery.
...
- `drivers: { [driverId]: { devices: [{...}] } }` — declared output-driver HSMs. Device shape varies per driver:
  ...
  - **PoKeys** (`address`-shape) — multi-device, addressed by uint serial number stored as a string in `address`. Each device carries `{ address, name, pwmPeriodMicroseconds, digitalOutputs: [{pin, invert}], pwmOutputs: [{channel}], extBusOutputs: [{bit, invert}] }`. PoExtBus bits are 1..80 flat (1=Device1:A, 8=Device1:H, 9=Device2:A …) — the editor UI shows the Device/Letter form for orientation against the PoKeys vendor tool but stores the flat bit number on disk and at runtime. Single catalog id `pokeys`; the kind-mismatch validator dispatches per-channel via `DRIVER_CHANNEL_KIND.pokeys` (a function — `analog` for `PWM[N]`, `digital` otherwise). Root element `<PoKeys>`. Per-device element order matches C# class declaration: Serial, Name, PWMPeriodMicroseconds, DigitalOutputs (`<Output>` with `<Pin>1..55</Pin>` and `<Invert>` bool), PWMOutputs (`<Output>` with `<Channel>1..6</Channel>` — 1-based, where PWM1=physical pin 17 … PWM6=physical pin 22; the C# HSM applies the DLL's reverse-channel-indexing internally so the editor and config stay human-readable), PoExtBusOutputs (`<Output>` with `<Bit>1..80</Bit>` and `<Invert>` bool). PWM period is per-device because the hardware shares one period across all 6 PWM channels; stored in microseconds so the file is portable across PoKeys55 (12 MHz clock) and PoKeys56/57 (25 MHz clock). Defaults filled by `backfillPoKeysDevices`.
...
- When adding a new output driver: add a row to `DRIVER_PATTERNS` (regex + parser + class FQN) **and** to `DRIVER_META` (label + configFilename + deviceShape + defaultDevice). The parser, save flow, and Hardware tab UI all key off these tables. If the driver carries both analog and digital outputs (PoKeys is the precedent — digital pins, PWM channels, PoExtBus bits all on one board), keep it as a SINGLE driver id and set `DRIVER_CHANNEL_KIND[driverId]` to a function `(channel) => 'analog'|'digital'` that dispatches on the channel string format. Call sites use `getChannelKind(driver, channel)` which transparently handles both flat-string entries (most drivers) and per-channel-function entries.
