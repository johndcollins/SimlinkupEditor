...
function gaugeInputRange(pn, gaugePort) {
  ...
  //
  // For normalized-input channels (-1..+1, the HSI CDI's signature) the
  // live-cal slider deliberately speaks the SAME axis as the breakpoint
  // table — so a slider value of 0.294 means "294% of full-scale" and
  // maps to whatever the user wrote at input=0.294 in the table. The
  // bridge converts back to F4's native degrees by multiplying by the
  // current limit signal (FalconBridge handles this for
